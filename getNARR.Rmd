---
title: "Download NARR Data Associated With Big Tornado Days"
author: "James Elsner"
date: "11/1/2018"
...
Download the grib files. ~ 2 hours to download 300 grb files from home.
```{r, eval=FALSE}
...
df %>%
  filter(nT >= 30) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(sqrt(aUSTM^2 + aVSTM^2))) %>%
...
summary(lmer(log(AvgATE) ~ scale(X) + nT + I(Yr > 2007) + I(Yr - 2004) + (1|Month), 
             weights = nT, 
             data = df[df$nT >= 20, ]))

summary(lmer(log(AvgATE) ~ scale(X) + nT + I(mCAPE/1000) * I(sqrt(aVSTM^2 + aUSTM^2)) + I(Yr > 2007) + I(Yr - 2004) + (1|Month),
             weights = nT, 
             data = df[df$nT >= 20, ]))
summary(model2)
AIC(model2)

model3 <- lmer(log(ATE) ~ scale(X) + scale(Y) + ENSO + aUSTM * I(Yr - 2004) + (1|Month), 
                weights = nT, 
                data = df)
summary(model3)
