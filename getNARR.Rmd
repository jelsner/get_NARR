...
Use a projection that matches the environmental data raster.
```{r}
...
summary(lmer(I(mCAPE/10) ~ I(Yr - 2004) + (1|Month), weights = nT, data = df[df$Yr >= 2005,]))

model0a <- lmer(log(ATE) ~ I(mCAPE/1000) + I(mUSTM/10) + I(Yr - 2004) + (1|Month), 
                weights = nT, 
                data = df)
summary(model0a)
AIC(model0a)

model0b <- lmer(log(ATE) ~ I(mCAPE/1000) + I(mCIN/100) + I(mUSTM/10) + I(Yr - 2004) + (1 |Month), 
                weights = nT, 
                data = df)
summary(model0b)
AIC(model0b)
