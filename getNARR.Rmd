...
df %>%
  filter(nT >= 20) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(mCAPE * aUSTM)) %>%
...
summary(lmer(log(ATE) ~ scale(X) + scale(Y) + ENSO + I(Yr - 2004) + (1|Month), 
                weights = nT, 
                data = df[df$nT >= 50, ]))

model2 <- lmer(log(ATE) ~ scale(X) + scale(Y) + ENSO + I(mCAPE/1000) * aVSTM + I(Yr - 2004) + (1|Month), 
                weights = nT, 
                data = df[df$nT >= 20, ])
summary(model2)
AIC(model2)
